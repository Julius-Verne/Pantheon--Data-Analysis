---
title: "Popular Historic Figures Exploration"
author: by Julian Hernández
output:
  html_document:
    toc: true
    toc_depth: 3
...
```{r fig.height=15, occupation_count}
...
followed by Barack Obama (200 languages) and US actor Corbin Bleu (193), which is
honestly a surprise since his biggest movie is High School Musical
...
the investigation. How did the feature(s) of interest vary with other features in the dataset?
...
####**Conclusion**: 
The behavior we saw mean women tend to be more popular
...
####**In Conclusion,**
It is no secret that women have had the short end of the 
...
Through the analysis I found variables that were good candidates for the model: \
...
p4 <- ggplot(db, aes(x = sex, y = article_languages)) + 
  geom_boxplot(aes( color = sex))+
  ggtitle("Languages", subtitle = "Up to the 0.9 quantile")+ 
...
age <- ggplot(db, aes(x = sex, y = antiquity)) + 
  geom_boxplot(aes( color = sex))+
  ggtitle("Antiquity", subtitle = "Up to the 0.9 quantile")+ 
  labs(colour = "Cylinders", x = "Sex", y = "Years")+
  ylim(0, quantile(db$antiquity, probs = 0.90)) 


grid.arrange(women, p1,p2, p4, age, p3, ncol = 2, nrow = 3, top = "Sex and ...")
...
**About the Graph:** In most boxplots we dont't see the complete dataset but up 
to the 90 quantile, this is due to the amount of ouliers in the datatset. Without
ignoring this outliers the boxplots are not easy to read, by doing so we gain a 
better understanding of the distribution of each variable even if we are missing
some values.
...
Personally I would like to know what drives great people, are there more people
in a certain country after they win a war or does a country inspire their 
people if they are doing well economically. It would be also interesting to do
this sort of analysis but with teams or groups that are well known, not just 
individuals.
